...
import {
  ...
  getEmissionSourceByFacilityId
} from "../services/EmissionSourceService";
...
export const EmissionSourceProvider = ({ children }) => {
  ...
  const fetchEmissionSourceByFacilityId = async (id) => {
    try {
      setLoading(true);
      const emissionSource = await getEmissionSourceByFacilityId(id);
      return emissionSource;
    } catch (error) {
      console.error("Error fetching emission source by id:", error);
    } finally {
  ...
  const addEmissionSource = async (emissionSourceData) => {
    try {
      ...
      setEmissionSources((prevEmissionSource) => [
        ...prevEmissionSource,
        newEmissionSource,
      ]);
  ...
  const editEmissionSource = async (id, emissionSourceData) => {
    try {
      setLoading(true);
      const updatedEmissionSource = await updateEmissionSource(
        id,
        emissionSourceData
      );
      setEmissionSources(
  ...
  const removeEmissionSource = async (id) => {
    try {
      ...
      setEmissionSources(
        emissionSources.filter((emissionSource) => emissionSource.id !== id)
      );
  ...
  return (
    <EmissionSourceContext.Provider
      value={{
        emissionSources,
        loading,
        fetchAllEmissionSources,
        fetchEmissionSourceById,
        addEmissionSource,
        editEmissionSource,
        removeEmissionSource,
        fetchEmissionSourceByFacilityId
      }}
    >
      {children}
    </EmissionSourceContext.Provider>
