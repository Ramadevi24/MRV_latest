...
import { getSubPlant, getSubPlantById, createSubPlant, updateSubPlant, deleteSubPlant  } from "../services/SubPlantService";
    ...
    const fetchAllSubPlants = async () => {
        ...
        const editSubPlant = async (id, subPlantData) => {
            try {
                ...
                setSubPlants(
                    subPlants.map((subPlant) => subPlant.id === id ? updatedSubPlant : subPlant)
                    );
                } catch (error) {
          ...
          const removeSubPlant = async (id) => {
            try {
                ...
                return (
                    <SubPlantContext.Provider value={{subPlants, loading, fetchAllSubPlants, fetchSubPlantById, addSubPlant, editSubPlant, removeSubPlant,}}> {children} </SubPlantContext.Provider>
                );
          };           
